...
import kevin.utils.PlaceInfo.Companion.get
...
class OldScaffold : Module("OldScaffold", "scaffolding", category = ModuleCategory.WORLD) {
    ...
    private val placeDelayTower = BooleanValue("PlaceableDelayWhenTowering", true)
    ...
    private val advancedYawModeValue = ListValue("AdvancedYawRotations", arrayOf("Offset", "Static", "RoundStatic", "Vanilla", "Round", "MoveDirection", "OffsetMove"), "MoveDirection")
    private val advancedPitchModeValue = ListValue("AdvancedPitchRotations", arrayOf("Offset", "Static", "Vanilla"), "Static")
    private val advancedYawOffsetValue = IntegerValue("AdvancedOffsetYaw", -15, -180, 180)
    private val advancedYawMoveOffsetValue = IntegerValue("AdvancedMoveOffsetYaw", -15, -180, 180)
    private val advancedYawStaticValue = IntegerValue("AdvancedStaticYaw", 145, -180, 180)
    private val advancedYawRoundValue = IntegerValue("AdvancedYawRoundValue", 45, 0, 180)
    private val advancedPitchOffsetValue = FloatValue("AdvancedOffsetPitch", -0.4f, -90f, 90f)
    private val advancedPitchStaticValue = FloatValue("AdvancedStaticPitch", 82.4f, -90f, 90f)
    ...
    private val zitterSpeedValue = FloatValue("ZitterSpeed", 0.13f, 0.1f, 0.3f)
    private val zitterStrengthValue = FloatValue("ZitterStrength", 0.072f, 0.05f, 0.2f)

    // Game
    private val timerValue = FloatValue("Timer", 1f, 0.1f, 5f)
    private val motionSpeedEnabledValue = BooleanValue("MotionSpeedSet", false)
    ...
    private val stopWhenBlockAboveValue = BooleanValue("StopTowerWhenBlockAbove", true)
    private val towerFakeJumpValue = BooleanValue("TowerFakeJump", true)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (towerStatus || mc.thePlayer.isCollidedHorizontally) {
            ...
            when (sameYValue.get().lowercase()) {
                ...
                "whenspeed" -> {
                    canSameY = KevinClient.moduleManager.getModule(Speed::class.java)!!.state
                }
                else -> {
                    canSameY = false
                }
        ...
        if (mc.thePlayer.onGround) {
            ...
            if (zitterModeValue.equals("teleport")) {
                ...
                val yaw = Math.toRadians(mc.thePlayer.rotationYaw + if (zitterDirection) 90.0 else -90.0).toFloat()
        ...
        when (towerModeValue.get().lowercase()) {
            ...
            "packet" -> {
                if (mc.thePlayer.onGround && towerTimer.hasTimePassed(2)) {
                    ...
                    mc.netHandler.addToSendQueue(C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + 0.753, mc.thePlayer.posZ, false))
    ...
    fun onRender2D(event: Render2DEvent) {
        if (counterDisplayValue.get()) {
            ...
            RenderUtils.drawRectRoundedCorners(
                (width - w2 - 20) / 2.0,
                height * 0.8 - 24.0,
                (width + w2 + 18) / 2.0,
                height * 0.8 + 12.0,
            ...
            mc.ingameGUI.drawCenteredString(mc.fontRendererObj, "▼",width / 2.0f + 2f, height * 0.8f+8f, Color(43,45,48).rgb)
            var stack = barrier
            if (slot != -1) {
            ...
            mc.ingameGUI.drawCenteredString(mc.fontRendererObj, info, width / 2f, height * 0.8f, Color.WHITE.rgb, false)
    ...
    private fun search(blockPosition: BlockPos, checks: Boolean): Boolean {
        ...
        if (!towerrotationsValue.equals("None") && towerStatus) {
            lockRotation = when (towerrotationsValue.get().lowercase()) {
                ...
                "custom" -> {
                    Rotation(mc.thePlayer.rotationYaw + customtowerYawValue.get(), customtowerPitchValue.get())
                }
                "advanced" -> {
                    var advancedYaw = 0f
        ...
        if (!rotationsValue.equals("None") && !towerStatus) {
            lockRotation = when (rotationsValue.get().lowercase()) {
                ...
                "custom" -> {
                    Rotation(mc.thePlayer.rotationYaw + customYawValue.get(), customPitchValue.get())
                }
                "better" -> {
                    Rotation(mc.thePlayer.rotationYaw + customYawValue.get(), placeRotation.rotation.pitch)
                }
